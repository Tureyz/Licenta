...
{
	glm::vec3 p1 = m_points[0]->m_projection, p2 = m_points[1]->m_projection, p3 = m_points[2]->m_projection, p4 = m_points[3]->m_projection;
	...
	float kp = 1.f - std::powf(1.f - m_stiffness, 1.f / iterationCount);
	float term = std::sqrt(1.f - d * d) * (std::acos(d) - m_initialAngle) * kp;
