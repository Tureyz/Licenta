...
namespace CudaMassSpring
{
	...
	__global__ void ClothEngineStep(const int particleCount, float3 * __restrict__ positions, float3 * __restrict__ prevPositions, float3 * __restrict__ velocities,
		...
		const int * __restrict__ springInfo, const int * __restrict__ springIDs, const float3 gravity, const float dampingCoef,
		const float springDampingCoef,
		const float timeStep, const float bendCoef,
		const bool * __restrict__ fixedVerts);



	void AdjustSprings(thrust::device_vector<float3> &positions, const thrust::device_vector<float3> &prevPositions,
		thrust::device_vector<float3> &velocities, 
		const thrust::device_vector<int> &bIDs,
